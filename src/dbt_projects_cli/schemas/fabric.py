...
class GitPackage(BaseModel):
    ...
    subdirectory: Optional[str] = Field(None, description="Subdirectory within the git repository")
    revision: Optional[str] = Field(None, description="Git revision (branch, tag, or commit)")
    warn_unpinned: Optional[bool] = Field(True, description="Whether to warn about unpinned packages")
...
class DatabricksConnection(BaseModel):
    ...
    def validate_host(cls, v: str) -> str:
        ...
        if not v.endswith('.cloud.databricks.com') and not v.endswith('.azuredatabricks.net'):
...
class ProjectConfig(BaseModel):
    ...
    schema_name: str = Field(..., alias="schema", description="Databricks schema name for this project")
    ...
    catalog: Optional[str] = Field(None, description="Optional project-specific catalog override")
...
class FabricConfig(BaseModel):
    """Root configuration model for lightweight fabric deployment with multiple projects."""
    
    fabric: FabricMetadata = Field(..., description="Fabric metadata")
    databricks: DatabricksConnection = Field(..., description="Databricks connection details")
    tags: Optional[List[str]] = Field(default_factory=list, description="Fabric-level tags")
    projects: Dict[str, ProjectConfig] = Field(..., description="Dictionary of project configurations")
    
    @field_validator('projects')
    @classmethod
    def validate_projects_not_empty(cls, v: Dict[str, ProjectConfig]) -> Dict[str, ProjectConfig]:
...
class MultipleFabricsConfig(BaseModel):
    ...
    fabrics: Dict[str, FabricConfig] = Field(..., description="Dictionary of fabric configurations")
    
    @field_validator('fabrics')
    @classmethod
    def validate_fabrics_not_empty(cls, v: Dict[str, FabricConfig]) -> Dict[str, FabricConfig]:
    ...
    @model_validator(mode='after')
    def validate_fabric_names_match_keys(self):
        ...
        for key, fabric_config in self.fabrics.items():
            if fabric_config.fabric.name != key:
                raise ValueError(f"Fabric name '{fabric_config.fabric.name}' does not match key '{key}'")
        return self
...
class LegacyFabricConfig(BaseModel):
    ...
    schema_name: Optional[str] = Field(None, alias="schema", description="Legacy schema field")
