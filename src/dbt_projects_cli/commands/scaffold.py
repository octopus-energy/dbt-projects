...
DATA_MESH_ALIGNMENTS = {
    "source-aligned": {
        "description": "Data products that extract and model data from specific source systems",
        "path_pattern": "packages/domains/source-aligned/{source_system}/{domain_name}",
        "naming_pattern": "{source_system}_{domain_name}",
        "examples": ["databricks_systems_data", "salesforce_customer_data", "stripe_payment_data"]
    },
    "consumer-aligned": {
        "description": "Data products that serve specific business use cases or consumer needs", 
        "path_pattern": "packages/domains/consumer-aligned/{business_area}/{use_case}",
        "naming_pattern": "{business_area}_{use_case}",
        "examples": ["marketing_customer_analytics", "finance_revenue_reporting", "operations_supply_chain"]
    },
    "utils": {
...
SOURCE_SYSTEMS = [
    "databricks", "salesforce", "stripe", "hubspot", "zendesk", 
    "postgresql", "mongodb", "kafka", "elementary", "google_analytics",
    "facebook_ads", "google_ads", "intercom", "segment"
]

# Known business areas  
BUSINESS_AREAS = [
    "marketing", "finance", "operations", "customer_success", 
    "product", "engineering", "hr", "legal", "compliance"
]
...
def package(name, description, profile):
    ...
    if not description:
        description = Prompt.ask("[bold cyan]Package description[/bold cyan]", default="")
    ...
    console.print(f"[bold green]Creating package '{name}' at {package_path}[/bold green]")
    ...
    console.print(f"  2. Configure your profile in profiles.yml")
    console.print(f"  3. Start building your models in models/")
...
def info() -> None:
    ...
    for alignment, config in DATA_MESH_ALIGNMENTS.items():
        ...
        table.add_row(
            config["path_pattern"],
            f"{config['description']} ({alignment})",
            ", ".join(config["examples"][:3]) + "..."
        )
...
@scaffold.command()
@click.option('--alignment', '-a', type=click.Choice(['source-aligned', 'consumer-aligned', 'utils']), help='Data mesh alignment type')
...
def domain(alignment, source_system, business_area, domain_name, description):
    ...
    if not alignment:
        ...
        for align_type, config in DATA_MESH_ALIGNMENTS.items():
            console.print(f"[bold cyan]{align_type}[/bold cyan]: {config['description']}")
    ...
    if alignment == "source-aligned":
        if not source_system:
            console.print("[yellow]Available source systems:[/yellow] " + ", ".join(SOURCE_SYSTEMS))
            source_system = Prompt.ask("[bold cyan]Source system[/bold cyan]")
            
            if source_system not in SOURCE_SYSTEMS:
                add_new = Confirm.ask(f"'{source_system}' is not in our standard list. Add it anyway?")
                if not add_new:
                    console.print("[red]Cancelled[/red]")
                    return
        
        if not domain_name:
            domain_name = Prompt.ask("[bold cyan]Domain name (e.g., 'systems-data', 'customer-data')[/bold cyan]")
        
        package_name = f"{source_system}_{domain_name.replace('-', '_')}"
        package_path = Path("packages/domains/source-aligned") / source_system / domain_name
        
    elif alignment == "consumer-aligned":
        if not business_area:
            console.print("[yellow]Available business areas:[/yellow] " + ", ".join(BUSINESS_AREAS))
            business_area = Prompt.ask("[bold cyan]Business area[/bold cyan]")
            
            if business_area not in BUSINESS_AREAS:
                add_new = Confirm.ask(f"'{business_area}' is not in our standard list. Add it anyway?")
                if not add_new:
                    console.print("[red]Cancelled[/red]")
                    return
        
        if not domain_name:
            domain_name = Prompt.ask("[bold cyan]Use case name (e.g., 'customer-analytics', 'revenue-reporting')[/bold cyan]")
        
        package_name = f"{business_area}_{domain_name.replace('-', '_')}"
        package_path = Path("packages/domains/consumer-aligned") / business_area / domain_name
        
    elif alignment == "utils":
        if not domain_name:
            domain_name = Prompt.ask("[bold cyan]Utility type (e.g., 'core', 'pii', 'testing')[/bold cyan]")
        
        package_name = f"utils_{domain_name.replace('-', '_')}"
        package_path = Path("packages/utils") / domain_name
    
    if not description:
        description = Prompt.ask("[bold cyan]Package description[/bold cyan]", default=f"Data domain package for {alignment} alignment")
    ...
    expected_name = DATA_MESH_ALIGNMENTS[alignment]["naming_pattern"].format(
        source_system=source_system if alignment == "source-aligned" else "",
        domain_name=domain_name.replace("-", "_") if alignment != "consumer-aligned" else "",
        business_area=business_area if alignment == "consumer-aligned" else "",
        use_case=domain_name.replace("-", "_") if alignment == "consumer-aligned" else "",
        utility_type=domain_name.replace("-", "_") if alignment == "utils" else ""
    )
    
    if package_name != expected_name:
        console.print(f"[yellow]⚠️  Package name '{package_name}' doesn't follow expected pattern '{expected_name}'[/yellow]")
        if not Confirm.ask("Continue anyway?"):
            console.print("[red]Cancelled[/red]")
            return
    
    console.print(f"[bold green]Creating {alignment} domain package '{package_name}' at {package_path}[/bold green]")
    ...
    if alignment == "source-aligned":
        template_context.update({
            ...
            "group_description": f"{source_system.title()} system data group"
        })
    elif alignment == "consumer-aligned":
        template_context.update({
            "business_area": business_area,
            "group_name": business_area,
            "group_description": f"{business_area.title()} analytics group"
        })
    elif alignment == "utils":
        template_context.update({
    ...
    _create_domain_structure(package_path, package_name, description, alignment, template_context)
    
    console.print(f"[bold green]✅ Domain package '{package_name}' created successfully![/bold green]")
    console.print(f"[yellow]Next steps:[/yellow]")
    console.print(f"  1. cd {package_path}")
    console.print(f"  2. Review and customize the generated configuration")
    console.print(f"  3. Start building your domain models in models/")
    console.print(f"[dim]💡 This package follows {alignment} data mesh principles[/dim]")
...
def fabric(fabric, project, component, catalog, workspace_id):
    """Create a new fabric project in the fabrics/ directory."""
    
    if not fabric:
        fabric = Prompt.ask("[bold cyan]Fabric name[/bold cyan]", default="oeuk-data-prod")
    ...
    if not catalog:
        catalog = Prompt.ask("[bold cyan]Databricks catalog[/bold cyan]", default="octoenergy_data_prod_prod")
    
    if not workspace_id:
        workspace_id = Prompt.ask("[bold cyan]Databricks workspace ID[/bold cyan]", default="337210421809158")
    ...
    # Create directory structure
    _create_fabric_structure(fabric_path, fabric, project, component, catalog, workspace_id)
    
    console.print(f"[bold green]✅ Fabric project created successfully![/bold green]")
    console.print(f"[yellow]Next steps:[/yellow]")
    console.print(f"  1. cd {fabric_path}")
    console.print(f"  2. Configure your Databricks connection")
    console.print(f"  3. Start building your models in models/")


def _create_package_structure(package_path: Path, name: str, description: str, profile: str):
...
"""
    ...
    (package_path / ".user.yml").write_text("# Add your user-specific configurations here\n")


def _create_fabric_structure(fabric_path: Path, fabric: str, project: str, component: str, catalog: str, workspace_id: str):
...
# This setting configures which "profile" from the profiles.yml dbt uses for this project.
profile: '{project_name}'

# These configurations specify where dbt should look for different types of files.
model-paths:
  - models
  # Although the groups directory doesn't contain any models its required for dbt to find the yml file for groups  
  - groups
analysis-paths:
  - analyses
test-paths:
...
"""
    ...
    # Create .user.yml placeholder
    (fabric_path / ".user.yml").write_text("# Add your user-specific configurations here\n")


def _create_domain_structure(package_path: Path, package_name: str, description: str, alignment: str, context: Dict[str, Any]):
    ...
    readme_content = _generate_domain_readme(package_name, description, alignment, context)
    (package_path / "README.md").write_text(readme_content)
    ...
    console.print(f"[bold green]✅ Domain structure for '{package_name}' created successfully![/bold green]")


def _generate_domain_dbt_project(package_name: str, alignment: str, context: Dict[str, Any]) -> str:
  ...
  {package_name}:
    ...
    staging:
      ...
      +access: private  # Staging models are private implementation details"""
        ...
        schema_name = f"{context.get('business_area', 'analytics')}_{context.get('domain_name', 'data').replace('-', '_')}"
        model_config = f"""
  {package_name}:
...
def _generate_domain_readme(package_name: str, description: str, alignment: str, context: Dict[str, Any]) -> str:
    """Generate README.md content for domain packages."""
    
    # Alignment-specific content
    alignment_info = {
        "source-aligned": {
            "purpose": f"This is a **source-aligned** data product that extracts and models data from the {context.get('source_system', 'unknown')} system.",
            "principles": """### Data Mesh Principles

- **Domain Ownership**: This package is owned by the team responsible for the {source_system} system
- **Data as a Product**: Provides clean, documented, and reliable data products from {source_system}
- **Self-Serve Infrastructure**: Can be independently deployed and maintained
- **Federated Governance**: Follows Octopus Energy's data standards and governance""".format(source_system=context.get('source_system', 'source system'))
        },
        "consumer-aligned": {
            "purpose": f"This is a **consumer-aligned** data product that serves the {context.get('business_area', 'business')} team's {context.get('domain_name', 'analytics')} use case.",
            "principles": """### Data Mesh Principles
...
- **Self-Serve Infrastructure**: Can be independently deployed and maintained  
- **Federated Governance**: Follows Octopus Energy's data standards and governance""".format(business_area=context.get('business_area', 'business'))
        },
        "utils": {
            "purpose": "This is a **utility package** that provides shared functionality across multiple data domains.",
            "principles": """### Data Mesh Principles
...
def _generate_domain_groups(package_name: str, alignment: str, context: Dict[str, Any]) -> str:
...
def _generate_domain_profiles(package_name: str, alignment: str, context: Dict[str, Any]) -> str:
    """Generate profiles.yml template for domain packages."""
    
    if alignment == "source-aligned":
        schema_prefix = context.get('source_system', 'unknown')
    elif alignment == "consumer-aligned":
        schema_prefix = f"{context.get('business_area', 'analytics')}_{context.get('domain_name', 'data').replace('-', '_')}"
    elif alignment == "utils":
        schema_prefix = "utils"
...
def _create_domain_example_models(package_path: Path, package_name: str, alignment: str, context: Dict[str, Any]):
...
-- It should extract and lightly transform data from {context.get('source_system', 'the source system')}
...
-- Staging model for {context.get('business_area', 'business')} {context.get('domain_name', 'analytics')}
select
    'example_id' as id,
    'example_metric' as metric_value,
    current_timestamp() as created_at

-- This model stages data for the {context.get('domain_name', 'analytics')} use case
"""
        (package_path / "models/staging/stg_example_data.sql").write_text(staging_model)
...
-- Mart model for {context.get('business_area', 'business')} {context.get('domain_name', 'analytics')}
with staged_data as (
...
-- This mart serves the {context.get('domain_name', 'analytics')} use case for the {context.get('business_area', 'business')} team
"""
        (package_path / "models/marts/mart_example_analytics.sql").write_text(mart_model)
