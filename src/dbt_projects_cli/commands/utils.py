...
def validate() -> None:
    ...
    for project_type, projects_list in projects_info.items():
        for project in projects_list:
            ...
            console.print(f"[{color}]{status}[/{color}] {project_name} ({project_type})")
            
            if not has_config:
                console.print(f"  [yellow]Warning: Missing or invalid dbt_project.yml[/yellow]")
            if not has_profile:
                console.print(f"  [yellow]Warning: No profile specified[/yellow]")
            if not has_models:
                console.print(f"  [yellow]Info: No models found[/yellow]")
    
    console.print(f"\n[bold]Summary:[/bold] {valid_projects}/{total_projects} projects validated successfully")
            ...
            for dir_name in dirs_to_clean:
                ...
                if dir_path.exists():
                    try:
                        ...
                        cleaned_dirs.append(str(dir_path.relative_to(discovery.root_path)))
                        console.print(f"[green]✓[/green] Cleaned {dir_path.relative_to(discovery.root_path)}")
                    except Exception as e:
                        console.print(f"[red]✗[/red] Failed to clean {dir_path.relative_to(discovery.root_path)}: {e}")
    
    if cleaned_dirs:
        console.print(f"\n[bold green]Successfully cleaned {len(cleaned_dirs)} directories[/bold green]")
    else:
