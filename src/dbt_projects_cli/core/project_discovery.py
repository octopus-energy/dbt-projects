...
class ProjectDiscovery:
    ...
    def discover_all_projects(self) -> Dict[str, List[Dict[str, Any]]]:
        ...
        projects_info: Dict[str, List[Dict[str, Any]]] = {"packages": [], "fabrics": []}
    ...
    def _discover_projects_in_directory(
        ...
        for dbt_project_file in directory.rglob("dbt_project.yml"):
            ...
            if any(part in excluded_dirs for part in dbt_project_file.parts):
                continue

            # Skip if it's not actually a file (could be a directory with the same name)
            ...
            try:
                ...
                console.print(
                    f"[yellow]Warning: Could not analyze project at "
                    f"{dbt_project_file}: {e}[/yellow]"
                )
    ...
    def _analyze_project(
        ...
        try:
            ...
            model_count = self._count_files_in_paths(
                project_dir, config.get("model-paths", ["models"]), [".sql"]
            )
            macro_count = self._count_files_in_paths(
                project_dir, config.get("macro-paths", ["macros"]), [".sql"]
            )
            test_count = self._count_files_in_paths(
                project_dir, config.get("test-paths", ["tests"]), [".sql"]
            )
