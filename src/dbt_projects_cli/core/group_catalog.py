...
class GroupCatalog:
    ...
    def select_group_interactive(self) -> Optional[GroupConfig]:
        ...
        while True:
            try:
                choice = Prompt.ask(
                    f"Select a group (1-{len(groups_list) + 1})", default="1"
                )
                ...
                if 1 <= choice_num <= len(groups_list):
                    ...
                    console.print(
                        f"[green]Selected group: {selected_group.name}[/green]"
                    )
...
def get_group_config(
    ...
    if group_from_catalog:
        ...
        if catalog_group:
            console.print(
                f"[green]Using group '{group_from_catalog}' from catalog[/green]"
            )
            return catalog_group
        else:
            console.print(
                f"[red]Group '{group_from_catalog}' not found in catalog[/red]"
            )
