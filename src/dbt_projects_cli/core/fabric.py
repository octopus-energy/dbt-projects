...
class FabricDeployer:
    ...
    def load_config(self, config_path: Path) -> FabricConfig:
        ...
        if self._is_multiple_fabrics_config(config_data):
            raise ValueError(
                "Configuration file contains multiple fabrics. "
                "Use 'list-fabrics' command to see available fabrics, "
                "then specify a fabric name with the '--fabric' option."
            )
    ...
    def load_fabric_from_multiple(
        self, config_path: Path, fabric_name: str
    ) -> FabricConfig:
        ...
        if not self._is_multiple_fabrics_config(config_data):
            raise ValueError(
                "Configuration file does not contain multiple fabrics. "
                "Use the standard commands without '--fabric' option."
            )
    ...
    def list_fabrics(self, config_path: Path) -> List[Dict[str, str]]:
        ...
        for name, fabric_config in multiple_config.fabrics.items():
            ...
            total_packages = sum(
                len(project.packages) for project in fabric_config.projects.values()
            )
            fabrics_info.append(
                {
                    "name": name,
    ...
    def generate_dbt_project(
        ...
        """Generate dbt project structure from fabric config.

        Generates a temporary dbt project for a specific project within the fabric.
    ...
    def _generate_dbt_project_yml(
        self, config: FabricConfig, project: ProjectConfig, output_dir: Path
    ) -> None:
        ...
        for package in project.packages:
            ...
            if isinstance(package, GitPackage):
                ...
                if package.warn_unpinned is not None:
                    package_dict["warn-unpinned"] = str(package.warn_unpinned)
    ...
    def _generate_profiles_yml(
        ...
        """Generate profiles.yml file for Databricks connection.

        Environment-specific settings are applied based on the environment parameter.
        """
        fabric_name_safe = config.fabric.name.replace("-", "_").replace(" ", "_")

        # Determine catalog name based on environment and project settings
        catalog_name = (
            project.catalog if project.catalog else config.get_catalog_name(environment)
        )
        ...
        if environment == "prod":
            outputs_dict = profile_config[fabric_name_safe]["outputs"]
            if isinstance(outputs_dict, dict) and environment in outputs_dict:
                env_config = outputs_dict[environment]
                    ...
                    env_config.update(
                        {
                            ...
                            "client_secret": (
                                '{{ env_var("DBT_DATABRICKS_CLIENT_SECRET") }}'
                            ),
        ...
        if config.databricks.http_path:
            ...
            if isinstance(outputs_dict, dict) and environment in outputs_dict:
                env_config = outputs_dict[environment]
                if isinstance(env_config, dict):
                    env_config["http_path"] = config.databricks.http_path
...
"""
    ...
    def _package_summary(
        self, package: Union[GitPackage, LocalPackage, HubPackage]
    ) -> str:
    ...
    def deploy_fabric_config(
        ...
        with Progress(
            ...
            if dry_run:
                ...
                self.console.print(
                    f"Packages: {len(project_config.packages)} configured"
                )
                catalog_name = (
                    project_config.catalog
                    if project_config.catalog
                    else config.get_catalog_name(environment)
                )
            ...
            if output_dir is None:
                output_dir = Path(
                    tempfile.mkdtemp(
                        prefix=f"fabric_{config.fabric.name}_{project_name}_"
                    )
                )
    ...
    def deploy(
        ...
        return self.deploy_fabric_config(
            config, project_name, output_dir, dry_run, environment
        )
