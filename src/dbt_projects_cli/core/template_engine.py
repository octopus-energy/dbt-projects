...
class ConfigTemplateEngine:
    ...
    def render_template(self, template_content: Any, context: Dict[str, Any]) -> Any:
        ...
        if isinstance(template_content, str):
            ...
            return {key: self.render_template(value, context) for key, value in template_content.items()}
        elif isinstance(template_content, list):
            return [self.render_template(item, context) for item in template_content]
        else:
            return template_content
    
    def generate_dbt_project_yml(self, context: Dict[str, Any], existing_config: Dict[str, Any] = None) -> str:
        ...
        # If we have existing config, merge it preserving manual additions
        if existing_config:
        ...
        yaml_content = yaml_content.replace("'{{ target.name != ''prod'' }}'", '"{{ target.name != \'prod\' }}"')
        
        # Fix template keys by replacing the literal template string with the actual package name
        yaml_content = yaml_content.replace("'{{ package_name }}':", f"{context['package_name']}:")
    ...
    def generate_packages_yml(self, context: Dict[str, Any]) -> str:
        ...
        return yaml.dump({'packages': packages}, default_flow_style=False)
    ...
    def get_directory_structure(self, alignment: str) -> List[str]:
        ...
        return self.config['directory_structures'].get(alignment, [])
    ...
    def validate_context(self, context: Dict[str, Any]) -> List[str]:
        ...
        for var_name, var_config in variables.items():
            ...
            if 'required_when' in var_config:
                ...
                if self._evaluate_condition(condition, context) and var_name not in context:
            ...
            if var_name in context and 'choices' in var_config:
                if context[var_name] not in var_config['choices']:
                    errors.append(f"Variable '{var_name}' must be one of: {var_config['choices']}")
        
        return errors
    
    def _evaluate_condition(self, condition: str, context: Dict[str, Any]) -> bool:
        """Simple condition evaluation (can be enhanced for more complex conditions)."""
    ...
    def _merge_configs(self, template_config: Dict[str, Any], existing_config: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        """Merge template configuration with existing configuration, preserving manual additions."""
        ...
        always_update_keys = {
            ...
            'model-paths', 'analysis-paths', 'test-paths', 'seed-paths', 
            'macro-paths', 'snapshot-paths', 'clean-targets'
        }
        ...
        if 'vars' in template_config:
            merged_config['vars'] = self._merge_vars_config(
                template_config['vars'], 
                existing_config.get('vars', {})
            )
        
        return merged_config
    
    def _merge_models_config(self, template_models: Dict[str, Any], existing_models: Dict[str, Any], context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if package_name and package_name in template_models:
            ...
            existing_package_config = merged_models.get(package_name, {})
    ...
    def _merge_package_model_config(self, template_config: Dict[str, Any], existing_config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in template_config.items():
            if key in template_managed_keys:
                ...
                for tag in template_tags:
                    if tag not in existing_tags:
                        existing_tags.append(tag)
                merged_config[key] = existing_tags
                ...
                merged_config[key] = self._merge_package_model_config(value, merged_config[key])
        
        return merged_config
    
    def _merge_vars_config(self, template_vars: Dict[str, Any], existing_vars: Dict[str, Any]) -> Dict[str, Any]:
        ...
        template_managed_vars = {
            'PROD_CATALOG', 'DATABRICKS_WORKSPACE_ID',
            'disable_dbt_artifacts_autoupload', 'disable_dbt_columns_autoupload',
            'disable_run_results', 'disable_tests_results', 'disable_dbt_invocation_autoupload',
            'distant_future_timestamp', 'local_timezone'
        }
