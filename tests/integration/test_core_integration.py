...
"""
    ...
    def test_project_discovery_integration(self, tmp_path):
        ...
        self.create_dbt_project_structure(tmp_path, "project1")
        self.create_dbt_project_structure(tmp_path, "project2")
    ...
    def test_model_analysis_integration(self, tmp_path):
        ...
        project_dir = self.create_dbt_project_structure(tmp_path, "analytics_project")

        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            # Get a model file path
            model_path = project_dir / "models" / "marts" / "user_metrics.sql"
            result = runner.invoke(cli, ["models", "analyze", str(model_path)])
    ...
    def test_model_listing_integration(self, tmp_path):
        ...
        self.create_dbt_project_structure(tmp_path, "data_project")

        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            result = runner.invoke(cli, ["models", "list", "--project", "data_project"])

        assert result.exit_code == 0

    def test_descriptions_generate_integration(self, tmp_path):
        """Test description generation through main CLI."""
        self.create_dbt_project_structure(tmp_path, "desc_project")
    ...
    def test_descriptions_show_models_integration(self, tmp_path):
        ...
        self.create_dbt_project_structure(tmp_path, "show_project")

        runner = CliRunner()
        with runner.isolated_filesystem(temp_dir=tmp_path):
            result = runner.invoke(
                cli, ["descriptions", "show-models", "--project", "show_project"]
            )
    ...
    def test_migration_dry_run_integration(self, tmp_path):
        ...
        project_dir = self.create_dbt_project_structure(tmp_path, "migrate_project")
    ...
    def test_error_handling_integration(self, tmp_path):
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            result = runner.invoke(
                cli, ["projects", "info", "--project", "nonexistent_project"]
            )
    ...
    def test_verbose_output_integration(self, tmp_path):
        ...
        self.create_dbt_project_structure(tmp_path, "verbose_project")
...
"""
    ...
    def test_end_to_end_workflow_integration(self, tmp_path):
        ...
        self.create_dbt_project_structure(tmp_path, "e2e_project")
        ...
        with runner.isolated_filesystem(temp_dir=tmp_path):
            ...
            # Get project info
            result = runner.invoke(
                cli, ["projects", "info", "--project", "e2e_project"]
            )
            # May fail but should handle gracefully

            # List models
            result = runner.invoke(cli, ["models", "list", "--project", "e2e_project"])
            assert result.exit_code == 0
