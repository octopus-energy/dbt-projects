...
class TestMigrationCommands:
    ...
    def create_test_package(self, package_path: Path, custom_config: dict = None):
        ...
        default_config = {
            "name": "test_package",
            "version": "1.0.0",
            "models": {"+tags": ["test_package"]},
            "vars": {"PROD_CATALOG": "old_catalog"},
        }
    ...
    def test_migrate_package_dry_run(self):
        ...
        self.create_test_package(
            package_path,
            {
                ...
                "models": {
                    ...
                    "custom_section": {"+materialized": "view"},
                },
        ...
        result = runner.invoke(
            cli,
            ["migrate", "package", "--package-path", str(package_path), "--dry-run"],
        )
    ...
    def test_migrate_package_preserves_custom_config(self):
        ...
        self.create_test_package(
            package_path,
            {
                ...
                "vars": {"custom_var": "custom_value", "marketing_threshold": 100},
            },
        )

        # Run migration
        result = runner.invoke(
            cli, ["migrate", "package", "--package-path", str(package_path), "--force"]
        )
        ...
        with open(package_path / "dbt_project.yml", "r") as f:
            ...
            assert (
                "custom_package_tag"
                in config["models"]["marketing_test_package"]["+tags"]
            )
            assert "custom_models" in config["models"]["marketing_test_package"]
            assert (
                config["models"]["marketing_test_package"]["custom_models"]["+meta"][
                    "owner"
                ]
                == "marketing_team"
            )
    ...
    def test_migrate_all_packages_dry_run(self):
        ...
        packages = [
            (
                "packages/domains/source-aligned/databricks/test-data",
                "databricks_test_data",
            ),
            (
                "packages/domains/consumer-aligned/marketing/analytics",
                "marketing_analytics",
            ),
        ...
        result = runner.invoke(cli, ["migrate", "all", "--dry-run"])
    ...
    def test_migrate_all_packages_by_domain_type(self):
        ...
        result = runner.invoke(
            cli, ["migrate", "all", "--domain-type", "consumer-aligned", "--force"]
        )
    ...
    def test_migrate_package_missing_path_error(self):
        ...
        result = runner.invoke(
            cli, ["migrate", "package", "--package-path", "nonexistent/path", "--force"]
        )
    ...
    def test_migrate_preserves_groups_yml(self):
        """Test that migration does not modify groups.yml (preserves
        existing config).
        """
        runner = CliRunner()

        # Create a test package with existing groups.yml
        package_path = Path("packages/domains/consumer-aligned/marketing/test-package")
        self.create_test_package(package_path, {"name": "marketing_test_package"})

        # Create existing groups file with custom content
        ...
        # Verify groups.yml was NOT modified (preserves existing config)
        assert groups_path.exists()
        assert groups_path.read_text() == original_content
        assert "custom_group" in groups_path.read_text()
